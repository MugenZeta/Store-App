    ...
    where TEntity : class, new()
{
    ...
    protected DomainServiceBaseComponentTests(DomainService<TEntity, TId> domainService,
        Expression<Func<TEntity, TId>> idExpression, EntityDataFactory<TEntity> entityDataFactory) : base(idExpression,
        entityDataFactory)
    {
    ...
    public virtual async Task GetById_ValidIdPassed_ShouldReturnResult()
    {
        ...
        var domainService = GetServiceInstance(async () => { await CreateTestEntityToStore(domain); });
    ...
    public virtual async Task Update_EmptyIdPassed_ShouldThrowException()
    {
        ...
        //Act + Assert
        await Assert.ThrowsAsync<InvalidLookupIdParameterException<TEntity>>(() =>
            domainService.Update(emptyIdValue, dummayObject));
    }
